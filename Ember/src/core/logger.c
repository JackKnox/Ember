...
void log_output(log_level level, const char* message, ...) {
    ...
    EM_ASSERT(level < EM_ARRAYSIZE(level_strings) && message != NULL && "Invalid arguments passed to log_output");
    ...
    emc_console_write("%s%s%s\033[0m\n", colors[level], level_strings[level], formatted);
