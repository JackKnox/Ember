...
VkResult vulkan_device_new_frame_semaphore(emgpu_device* device, vulkan_frame_context* frame_context, VkSemaphore* out_semaphore) {
    vulkan_context* context = (vulkan_context*)device->internal_context;

    u32 index = frame_context->semaphore_index;
    if (index >= darray_length(context->semaphore_pool)) {
        context->semaphore_pool = darray_resize(context->semaphore_pool, (index + 1) * DARRAY_RESIZE_FACTOR);

        for (u32 i = index; i < darray_capacity(context->semaphore_pool); ++i) {
            VkSemaphoreCreateInfo create_info = { VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO };

            VkResult result = vkCreateSemaphore(context->logical_device, &create_info,context->allocator, darray_push_empty(context->semaphore_pool));
...
em_result vulkan_renderpass_retrieve_framebuffer(
    emgpu_device* device, 
    ...
    create_info.attachmentCount = renderpass->attachment_count;
...
em_result vulkan_frame_context_init(const emgpu_frame* init_frame, vulkan_frame_context* out_context) {
    out_context->submissions = darray_create(vulkan_frame_submission, NULL, MEMORY_TAG_TEMP);
    out_context->managed_surfaces = darray_create(vulkan_frame_surface_entry, NULL, MEMORY_TAG_TEMP);
    out_context->wait_present_semaphores = darray_create(VkSemaphore, NULL, MEMORY_TAG_TEMP);
    out_context->frame_textures = mem_allocate(NULL, sizeof(emgpu_texture*) * (init_frame->current_resource_idx + 1), MEMORY_TAG_TEMP);
...
em_result vulkan_device_process_frame(emgpu_device* device, const emgpu_frame* frame, vulkan_frame_context* frame_context) {
    ...
    // --------------------------------------
    void* cursor = NULL;
    while (freelist_next_block(&frame->commands, &cursor)) {
        rendercmd_payload* payload = (rendercmd_payload*)cursor;
        ...
        switch (payload->hdr.type) {
            case RENDERCMD_SET_RENDERAREA: {
                ...
                if (payload->set_renderarea.set_scissor) {
                    VkRect2D scissor = {};
            ...
            case RENDERCMD_BIND_NEXT_SURFACE_TEXTURE: {
                ...
                vulkan_device_new_frame_semaphore(device, frame_context, &surface_entry->image_available_semaphore);
                ...
                vulkan_device_new_frame_semaphore(device, frame_context, &curr_submission->signal_semaphore);
            ...
            case RENDERCMD_BEGIN_RENDERPASS: {
                ...
                em_result result = vulkan_renderpass_retrieve_framebuffer(
