...
em_result vulkan_device_initialize(emgpu_device* device, em_allocator* allocator, const emgpu_device_config* config) {
    ...
    for (u32 i = 0; i < config->extension_count; ++i) {
        const emgpu_extension_desc* extension = &config->extensions[i];

        if (match_extension(extension, "EMBER_gpu_wayland_surface", EMBER_VERSION)) {
    ...
    for (u32 i = 0; i < physical_device_count; ++i) {
        ...
        for (u32 i = 0; i < queue_family_count; ++i) {
            ...
            // Present queue?
            if (context->vkGetPhysicalDevicePresentationSupportKHR(device, context->wsi_extension_data, physical_device, i)) {
