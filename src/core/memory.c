...
void* mem_allocate(em_allocator* allocator, u64 size, memory_tag tag) {
	em_allocator sys = em_allocator_default();
	if (!allocator) allocator = &sys;

	return em_memset(allocator->alloc(allocator, size, 0, tag), 0, size);
}
...
#endif
	return em_memset(allocator->realloc(allocator, block, old_size, new_size, 0, tag), 0, new_size);
}
